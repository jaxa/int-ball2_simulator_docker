	...
	dtc_(nh),seq_status_(0),valid_navigation_(false)
{
	...
	up_wrench_mode_sub_ = nh_.subscribe(TOPIC_UP_WRENCH_MODE, 1, &Ctl::upWrenchModeCallback, this);
	up_torque_gain_sub_ = nh_.subscribe(TOPIC_UP_TORQUE_GAIN, 1, &Ctl::upTorqueGainCallback, this);
...
//------------------------------------------------------------------------------
// Callback of publish on the TOPIC_UP_WRENCH_MODE
void Ctl::upWrenchModeCallback(const std_msgs::Int8ConstPtr& msg)
{
	try 
	{
		up_wrench_mode_ = static_cast<UP_WRENCH_MODE>(msg->data);
		ROS_INFO_STREAM("Wrench mode: " << int(up_wrench_mode_) << " at Ctl::upWrenchModeCallback");
	}
	catch (const std::exception& e) 
	{
		std::string what(ib2_mss::Log::caughtException(e.what()));
	...
	{
		ROS_ERROR("caught exception at Ctl::upWrenchModeCallback");
	}
}

//------------------------------------------------------------------------------
// Callback of publish on the TOPIC_UP_TORQUE_GAIN
void Ctl::upTorqueGainCallback(const std_msgs::Float64MultiArrayConstPtr& msg)
{
	try 
	{
		...
		up_torque_gain_ = vec;
		ROS_INFO_STREAM("Get Torque gain: [" << up_torque_gain_(0) << ","<< up_torque_gain_(1) << ","<< up_torque_gain_(2) << "]  at Ctl::upTorqueGainCallback");
	}
	catch (const std::exception& e) 
	{
		std::string what(ib2_mss::Log::caughtException(e.what()));
	...
	{
		ROS_ERROR("caught exception at Ctl::upTorqueGainCallback");
	}
}
...
int main(int argc, char **argv)
{
	...
	ros::init(argc, argv, "up_ctl_only");
    ros::NodeHandle nh_;
    Ctl Ctl(nh_);
    ros::spin();
	return 0;
